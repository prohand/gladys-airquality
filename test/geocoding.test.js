...
test('a town outside Europe is a place like any other', async () => {
  // Nothing in the geocoder knows about a country or a continent: this is what
  // makes the integration worldwide.
  stubFetch({
    results: [
      place('Tokyo', {
        latitude: 35.6895,
  ...
  const [tokyo] = await searchPlaces('Tokyo');
  assert.equal(tokyo.latitude, 35.6895);
  assert.equal(describePlace(tokyo), 'Tokyo (Tokyo, Japon)');
});

test('the answer comes back in the language of the device names', async () => {
  const calls = stubFetch({ results: [place('Munich')] });
  await searchPlaces('Munich', 'en');
...
test('accents and case are ignored when comparing', () => {
  ...
  assert.equal(filterByHints([place('Saint-Étienne')], ['saint-etienne']).length, 1);
});
...
test('two towns of the same name on two continents are never picked by coin flip', () => {
  // Picking one here would silently report another town's air.
  const candidates = [
    place('Paris', { country: 'France', admin2: 'Paris' }),
    place('Paris', { country: 'États-Unis', admin1: 'Texas', admin2: 'Lamar County' }),
  ];
  assert.equal(pickPlace(candidates, 'Paris'), null);
  assert.equal(pickPlace([], 'Paris'), null);
});
