...
//
// WHY THERE IS NO COUNTRY ANYWHERE. Everything downstream — the air quality
// provider, the index, the devices, the features — works on a latitude and a
// longitude. The country only ever existed here, as the registry that knew how
// to read a national postal code (France's API Géo), and that made the
// integration French while its data source covers the planet. One worldwide
// geocoder removes the step entirely: no country field in the form, no registry
// to extend, no manifest option list to keep in sync with the code.
//
// Source: the Open-Meteo geocoding API — the same open, key-free, account-free
// house as the air quality forecast this integration already reads, backed by
// the GeoNames database.
//
//   https://open-meteo.com/en/docs/geocoding-api
//
// A place name is not unique (there are several Montauban, a dozen Paris and
// four Springfield per state), so the search returns a LIST and the caller
// either finds ONE obvious answer or shows the candidates and asks the user to
// be more precise. The user narrows it down with commas — "Montauban,
// Tarn-et-Garonne", "Springfield, Illinois", "Nantes, 44000" — which is what
// `hints` below filter on; nothing is ever picked by coin flip, since the wrong
// pick silently reports another town's air.
// -----------------------------------------------------------------------------

import { createLogger } from '@gladysassistant/integration-sdk';
import { DEFAULT_LANGUAGE, normalizeText } from './language.js';
 ...
 * to the answers instead — region, state, country, postal code, in any order and
 * in any combination.
 * @param {string} query
...
/**
 * Search a place name, anywhere in the world.
 * @param {string} name
 * @param {string} [language] the language the place names come back in — a
 *   German user looking for "München" and an English one looking for "Munich"
 *   are asking the same question of the same database.
 * @returns {Promise<Array<object>>} the matches, best first (the API ranks them
 *   by relevance then by population)
 */
export async function searchPlaces(name, language = DEFAULT_LANGUAGE) {
...
function toPlaces(results) {
  return (Array.isArray(results) ? results : [])
    .map((result) => ({
      ...
      // admin1 is the region or state, admin2 the department or county, admin3
      // the district: what tells two towns of the same name apart.
 ...
 * misspelt region) empties the list rather than being ignored: "no place found
 * for what you typed" is honest, silently dropping the filter is not.
 ...
 * "Paris", "Parisot" and "Parisel" is the town they meant; "Paris" among the
 * French one and the Texan one is not, and asking beats guessing.
 * @param {Array<object>} places
 ...
 * @param {string} [language] language of the place names in the answer
 * @returns {Promise<{ match: object | null, candidates: Array<object> }>}
 */
export async function resolvePlace(query, language = DEFAULT_LANGUAGE) {
  const { name, hints } = splitQuery(query);
  const candidates = filterByHints(await searchPlaces(name, language), hints);
