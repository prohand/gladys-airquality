...
//
// A "location" is one place the user wants an air quality device for. The list
// is the single source of truth of the integration:
...
// `src/coordinates.js`: `Number('')` is 0, a valid latitude.
// -----------------------------------------------------------------------------
...
// Hard cap: each location is one device polling free public APIs. It is ALSO
// the number of options the delete action's dropdown offers — they are
// positions in this list, and a static manifest cannot offer more of them (a
// test keeps the two in sync).
export const MAX_LOCATIONS = 20;

// Long enough that two locations never collide, short enough that
// `ext:air-quality:air-quality-station:loc-3f8a2b1c` stays readable in a log.
const ID_LENGTH = 8;
...
/**
 * The location a 1-based POSITION designates — what the `location` select of
 * the delete action carries.
 *
 ...
 * the number the listing prints and the delete dropdown offers.
 */
...
export function upsertLocation(locations = [], patch = {}) {
  ...
  // location must not blank the commune it was resolved from.
 ...
 *
 * Used INSIDE a sentence ("Lieu 2 « Maison » ajouté : ..."), where the location
...
// newline into a space. A bare number does not survive that collapse —
// addresses are full of digits and dots — while a bullet cannot occur inside an
// address, which makes it the visible boundary between two entries whether the
// newline lives or dies.
 ...
 *
 * The listing puts the commune and the point in `detail`, the provider test
 * puts the air quality index there — so both answers read as the same list of
 * the same locations, and the number is the one the delete dropdown offers in
 * both.
 *
